...
- `SelectedItem setAndGetSelectedItem(InputCommand command)` - меняет и возвращает текущую опцию в зависимости от
  введенной команды
- `SelectedItem getCurrentItem()` - возвращает текущую опцию

##### class InteractionManager

Зона ответственности включает в себя все действия пользователя вне самого игрового процесса, то есть действия на
...
- `void processCommand(InputCommand command)` - данный метод вызывается каждый раз при введении пользователем той или
  ...
  показать правила игры, осуществить выход из приложения, начать игру, выделить другую опцию.
